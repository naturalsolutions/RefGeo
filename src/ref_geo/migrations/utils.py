...
def insert_areas_from_temporary_table(schema, temp_table_name, area_type):
    ...
    if geom_4326_exists():
        utils.insert_areas_from_temporary_table(op.get_bind(), schema, temp_table_name, area_type)
...
def insert_grids_from_temporary_table(schema, temp_table_name):
    logger.info("Copy grids in li_grids…")
    utils.insert_grids_from_temporary_table(op.get_bind(), schema, temp_table_name)
